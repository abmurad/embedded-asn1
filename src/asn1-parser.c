...
asn1_init(asn1_parser_t *parser, const uint8_t *data, size_t length) {
	EMBEDDED_ASSERT(parser != NULL);
	EMBEDDED_ASSERT(data != NULL);
...
asn1_next(asn1_parser_t *parser) {
	...
	*token       = (asn1_token_t){0};
	...
	if (token->type.tag == TYPE_MULTIPART_TAG) {
		...
		do {
			if (!advance_pos(parser, 1)) {
				return ERR(ERR_MALFORMED, TOKEN_TAG_TRUNCATED);
			}
			...
			if (bits > ASN1_TYPE_TAG_BITS) {
				return ERR(ERR_MEMORY, TOKEN_TAG_TOO_LARGE);
			}
	...
	// Length (8.1.3)
	if (!advance_pos(parser, 1)) {
		return ERR(ERR_MALFORMED, TOKEN_TRUNCATED_HEADER);
	}
	...
	if (CONTENT_LENGTH_IS_LONG_FORM(*parser->current)) {
		...
		if (num_bytes == CONTENT_LENGTH_LONG_RESERVED) {
			return ERR(ERR_MALFORMED, TOKEN_LENGTH_RESERVED);
		} else if (num_bytes == 0) {
			// Indefinite form is forbidden (X.690 11/2008 8.1.3.6)
			return ERR(ERR_MALFORMED, TOKEN_LENGTH_INDEFINITE);
		} else if (num_bytes > sizeof token->length) {
			return ERR(ERR_UNSUPPORTED, TOKEN_LENGTH_TOO_LONG);
		}

		for (i = 0; i < num_bytes; i++) {
			if (!advance_pos(parser, 1)) {
				return ERR(ERR_MALFORMED, TOKEN_LENGTH_TRUNCATED);
			}

			if (token->length == 0 && *parser->current == 0) {
				return ERR(ERR_MALFORMED, TOKEN_LENGTH_LEADING_ZERO);
			}
		...
		if (token->length < CONTENT_LENGTH_LONG_MIN) {
			return ERR(ERR_MALFORMED, TOKEN_LENGTH_BELOW_LONG_FORM_MIN);
		}
	} else {
	...
	if (token->length > 0) {
		...
		if (!advance_pos(parser, token->length)) {
			return ERR(ERR_MALFORMED, TOKEN_TRUNCATED_CONTENT);
		}
...
asn1_abort(asn1_parser_t *parser) {
	if (parser->depth == 0) {
		return ERR(ERR_INVALID, ABORT_ALREADY_AT_ROOT);
	}
...
asn1_push(asn1_parser_t *parser) {
	if (parser->token.type.encoding != ASN1_ENCODING_CONSTRUCTED) {
		return ERR(ERR_INVALID, PUSH_NOT_CONSTRUCTED);
	}
...
asn1_force_push(asn1_parser_t *parser) {
	...
	if (parser->depth + 1 > NUM(parser->stack)) {
		return ERR(ERR_UNSUPPORTED, PUSH_NESTED_TOO_DEEP);
	}
...
asn1_pop(asn1_parser_t *parser) {
	if (parser->depth == 0) {
		return ERR(ERR_INVALID, POP_ALREADY_AT_ROOT);
	}

	// Don't pop tokens which havent been fully parsed
	if (!asn1_eof(parser)) {
		return ERR(ERR_MALFORMED, POP_PARENT_NOT_FULLY_PARSED);
	}
...
asn1_push_seq(asn1_parser_t *parser) {
	...
	if (!asn1_is_sequence(&parser->token)) {
		return ERR(ERR_INVALID, PUSH_TAKEN_NOT_A_SEQUENCE);
	}
