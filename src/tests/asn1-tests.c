...
test_asn1_oid_decode(void) {
	...
	check_TRUE(asn1_oid_eq(&oid, TEST_OID1));

	check_OK(asn1_next(&parser));
	check_OK(asn1_oid(&parser.token, &oid));
	check_TRUE(asn1_oid_eq(&oid, TEST_OID2));

	check_OK(asn1_pop(&parser));
	check_TRUE(asn1_eof(&parser));
...
test_asn1_oid_decode_invalid(void) {
	...
	check_OK(asn1_next(&parser));
	check_EQ(asn1_oid(&parser.token, &oid), ERR_MALFORMED);
...
test_asn1_oid_to_string(void) {
	...
	check_EQ(asn1_oid_to_string(oid_str, sizeof(oid_str), &oid), 7);
	check_EQ(strncmp("1.2.123", oid_str, sizeof(oid_str)), 0);

	check_EQ(asn1_oid_to_string(oid_str, 6, &oid), 7);
	check_EQ(strncmp("1.2.1", oid_str, sizeof(oid_str)), 0);

	check_EQ(asn1_oid_to_string(oid_str, sizeof(oid_str), &empty), 0);
	check_EQ(strncmp("", oid_str, sizeof(oid_str)), 0);
...
test_asn1_oid_comparison(void) {
	...
	check_TRUE(asn1_oid_eq(&a, TEST_OID1));
	check_TRUE(!asn1_oid_eq(&b, TEST_OID1));

	check_EQ(asn1_oid_cmp(&a, &a), 0);
...
test_asn1_bitstring_decode_invalid(void) {
	...
	check_EQ(asn1_bitstring(&token2, buf, 0), ERR_MEMORY);
	check_EQ(asn1_bitstring(&token2, buf, sizeof buf), ERR_MALFORMED);
	check_EQ(asn1_bitstring(&token3, buf, sizeof buf), ERR_MALFORMED);
	check_EQ(asn1_bitstring(&token4, buf, sizeof buf), ERR_MALFORMED);
	check_EQ(asn1_bitstring(&token5, buf, sizeof buf), ERR_MALFORMED);
...
test_asn1_parse(void) {
	...
	// 0
	check_OK(asn1_next(&parser));
	check_TRUE(asn1_is_sequence(&parser.token));
	check_OK(asn1_push(&parser));
	...
	// 2
	check_OK(asn1_next(&parser));
	check_TRUE(asn1_is_int(&parser.token));
...
test_asn1_parse_longform(void) {
	...
	check_OK(asn1_next(&parser));
	check_EQ(parser.token.length, 128);
...
test_asn1_parse_single(void) {
	...
	check_TRUE(asn1_is_int(&parser.token));
	check_OK(asn1_int(&parser.token, &value));
	check_EQ(value, 1 - INT_MIN);
	check_TRUE(asn1_eof(&parser));

	asn1_init(&parser, raw2, sizeof raw2);
	check_OK(asn1_next(&parser));
	check_TRUE(asn1_is_null(&parser.token));
	check_TRUE(asn1_eof(&parser));
...
test_asn1_parse_invalid(void) {
	...
	check_EQ(asn1_next(&parser), ERR_MALFORMED);
	...
	check_EQ(asn1_next(&parser), ERR_UNSUPPORTED);
...
test_asn1_parse_time(void) {
	...
	check_EQ(asn1_time_cmp(&time, &TIME(1970, 1, 1, 0, 0, 0)), 0);

	check_OK(asn1_time(&y2k_token, &time));
	check_EQ(asn1_time_cmp(&time, &TIME(2000, 1, 1, 0, 0, 0)), 0);

	check_OK(asn1_time(&leap_feb_token, &time));
	check_EQ(asn1_time_cmp(&time, &TIME(2000, 2, 29, 0, 0, 0)), 0);

	check_OK(asn1_time(&y2k38_token, &time));
	check_EQ(asn1_time_cmp(&time, &TIME(2038, 1, 19, 3, 14, 8)), 0);
...
test_asn1_parse_invalid_time(void) {
	...
	check_EQ(asn1_time(&garbage_token, &time), ERR_MALFORMED);
	check_EQ(asn1_time(&incomplete_token, &time), ERR_MALFORMED);
	check_EQ(asn1_time(&missing_tz_token, &time), ERR_MALFORMED);
	check_EQ(asn1_time(&midnight_token, &time), ERR_MALFORMED);
	check_EQ(asn1_time(&leap_year_token, &time), ERR_MALFORMED);
	check_EQ(asn1_time(&days_token, &time), ERR_MALFORMED);
...
test_asn1_parse_invalid_int(void) {
	...
	check_EQ(asn1_int(&leading_ones_token, NULL), ERR_MALFORMED);
	check_EQ(asn1_int(&leading_zeroes_token, NULL), ERR_MALFORMED);
