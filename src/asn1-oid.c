...
asn1_oid(const asn1_token_t *token, asn1_oid_t *oid) {
	...
	*oid = (asn1_oid_t){0};
	...
	if (token->data == NULL || token->length == 0) {
		return ERR(ERR_MALFORMED, OID_ZERO_LENGTH);
	}
	...
	if ((*(token->data + token->length - 1) & OID_CONTINUATION_MASK) != 0) {
		return ERR(ERR_MALFORMED, OID_NO_END_MARKER);
	}
	...
	for (data = token->data; data < token->data + token->length; data++) {
		if (arc == 0 && *data == 0x80) {
			...
			// value 0x80"
			return ERR(ERR_MALFORMED, OID_LEADING_BYTE_IS_ZERO);
		}
		...
		if (arc_bits > sizeof(arc) * 8) {
			return ERR(ERR_MEMORY, OID_ARC_TOO_LONG);
		}
		...
		if ((*data & OID_CONTINUATION_MASK) == 0) {
			if (is_first_arc) {
				...
				if (!append_arc(oid, x)) {
					return ERR(ERR_MEMORY, OID_TOO_MANY_ARCS);
				}
